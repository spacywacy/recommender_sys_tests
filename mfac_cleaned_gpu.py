...
def batch_samp_negatives(user_batch, sample_size, exist_dict, all_items):
	...
	for user in user_batch.numpy():
		new_sample = sampling_negatives(user, sample_size, exist_dict, all_items)
		batch_samp = torch.cat([batch_samp, new_sample])

	return batch_samp

def pointwise_loss(net, users, items, exist_dict, all_items, users_non_cuda, sample_size):

	negatives_raw = batch_samp_negatives(users_non_cuda, sample_size, exist_dict, all_items)
	negatives = Variable(_gpu(negatives_raw))
...
def load_batch_static(in_col, batch_size):
	i = 0
	while i+batch_size <= len(in_col):
		yield in_col[i:i+batch_size]
		i+=batch_size

	if len(list(in_col)[i:]) > 0:
		yield in_col[i:]
...
def fit(loss_func,
	...
	epoch_losses = []
	all_items_in_numpy = items.numpy()
	users_cuda = _gpu(train_users)
	items_cuda = _gpu(train_items)

	for epoch in range(epoches):
		epoch_loss = 0.0
		n = 0

		batch_zip = zip(load_func(users_cuda, batch_size),
						load_func(items_cuda, batch_size),
						load_func(users, batch_size))
		for user_batch, item_batch, users_non_cuda in batch_zip:
			user_var = Variable(user_batch)
			item_var = Variable(item_batch)
			optimizer.zero_grad()
			loss = loss_func(net, user_var, item_var, exist_dict, all_items_in_numpy, users_non_cuda, sample_size)
...
def prediction_losses(test_set):
	test_users, test_items = split_columns(test_set)
	test_user_var = Variable(_gpu(test_users))
	test_item_var = Variable(_gpu(test_items))
	test_items_numpy = test_items.numpy()

	print('positive loss:', positive_loss(net_, test_user_var, test_item_var))

	exist_dict_test_ = construct_dict(test_set)
	pointwise_loss_ = pointwise_loss(net_, test_user_var, test_item_var, exist_dict_test_, test_items_numpy, test_users, 1)
	print('pointwise_loss:', pointwise_loss_)


print('g')
fname = 'ml-100k/u.data'
data = load_data(fname, size=12000)
...
net_ = _gpu(bilinear(1000, 1700, 5))
batch_size_ = 1024
epoches_ = 1200
wd_ = .1
...
fit(loss_func_,
	...
	sample_size_)
t1_fit = time()
print('training time:', t1_fit - t0_fit)
print('')


#pickle_dump('try_gpu_600_epoch.pickle', net_)
#net_ = pickle_load('try_weight_decay_1e-5_full_size.pickle')

prediction_losses(test_set)
